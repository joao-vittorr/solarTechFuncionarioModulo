...
@section('content')
    ...
    @if ($categorias && $categoria->id)
        <form id="main" method="POST" action="{{ route('categorias.update', ['id' => $categoria->id]) }}"
            enctype="multipart/form-data">
            @method('PUT')
        @else
            <form id="main" method="POST" action="{{ route('categoria.store') }}" enctype="multipart/form-data">
    @endif
    ...
    @csrf
        ...
        <div class="form-group">
            ...
            <input type="text" name="nome" id="nome" class="form-control"
                value="{{ optional($categoria) ? $categoria->nome : '' }}" required>
        </div>
